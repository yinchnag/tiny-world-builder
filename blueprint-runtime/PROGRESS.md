...
- [x] **EX-0 执行器接缝 + 激活 + 输入物化（地基）** ✅ — `runtime/engine/exec/`：`NodeExecutor` 接缝(async：node+物化输入→outputs+状态+事件)+ 注册表；`inbox.materializeInbox`(扫 message.delivered 按 in 口归集，靠 D1)；`run.runNode`(物化→跑执行器→输出沿出边 route 投递→推进状态→发 exec.*，emit 注入避免 engine→mcp 反依赖)；graph-tools `run_node` 工具(D4 触发即返回，结果经 SSE)。D1/D3 落地(delivered 带 nodeId/portId/payload；events 加 exec.started/completed/failed)。验收：runNode 全 loop(echo 执行器物化输入+输出投递+exec.*)、run_node 工具触发 ✓。
- [x] **EX-1 Terminal 执行器（确定性纵切）** ✅ — `runtime/engine/exec/terminal`：`terminalExecutor` 跑 `properties.command`(node:child_process)→ StdoutChunk/StderrChunk/ExitStatus + 状态 exited。验收：`echo hello`→stdout 'hello'+exit 0；`exit 3`→非零退出码(单测，无需 key)。**执行器模式端到端打通。** 沙箱硬化留后续。
- [x] **EX-2 ModelProvider 抽象 + OpenAI-compatible 供应商** ✅ — `runtime/engine/exec/providers`：`ModelProvider.complete` + OpenAI-compatible 适配器(裸 fetch 调 chat/completions，**一把覆盖 OpenAI/DeepSeek/Qwen**) + registry(名→baseURL/apiKeyEnv/defaultModel，`resolveProvider` 从 env 取 key)。验收：适配器对 mock fetch 产正确请求(url/model/auth)+解析；registry 解析/默认 model ✓。
- [x] **EX-3 Agent 执行器（核心目标）** ✅ — `runtime/engine/exec/agent`：`createAgentExecutor(resolve)`——物化 context/message/task → 构对话 → 调 ModelProvider → 产 AgentReport + 状态 reporting；未知供应商→exec.failed。验收：单测(mock provider，对话构造+输出)；**集成 `ex3-agent`：搭图→deliver 喂 context→run_node→Agent 调 mock provider→AgentReport 沿出边投递到下游 ✓** = 「运行一个通过工作流搭建的 Agent」(换 resolveProvider(env) 即真推理)。
...
- [x] **EX-5 editor 接线（用户面）** ✅ — runtime 服务(`runtime/mcp/server` + `pnpm serve`，CORS) + editor 接线(`sync/runtime-client` 组合 http-adapter+mcp-client；App 订阅 SSE + onRun 镜像全图→run_node) + 检视器(Agent 模型配置输入 + 「运行」按钮 + 运行状态/收到产出显示)。**联跑**：`$env:DEEPSEEK_API_KEY=...; pnpm serve`(8787) + 另窗 `pnpm --filter @blueprint/editor dev`(5173) → 浏览器建 Agent→设 provider→点运行→结果经 SSE 回显。验收：属性编辑/运行按钮 jsdom + 13 E2E ✓；真运行为双服务联跑手动验(浏览器 Playwright 双服务 E2E 留 EX-6)。
...
- 2026-06-30 **手动试跑脚本** `scripts/try-agent.ts`（`pnpm try:agent <provider> [model]`）：用真 env key 跑一次 Agent 执行器验证真推理（非 CI，需网络+key；走同一 createAgentExecutor+resolveProvider，仅把 mock 换成 env key）。无 key 时清晰报缺哪个环境变量。scripts/ 在顶层（非边界目录），tsx 直跑，不进 vitest/mirror/types。
